...
function test_octal
{
    ...
    for ((i = 0; i < ${#perms[@]}; i++)); do
        local num=$("$rbh_lfind" "$URI" -perm "${perms[i]}" | wc -l)

        (( "$num" == 1 )) ||
            error "$rbh_lfind -perm ${perms[i]}: $num != 1"

        num=$("$rbh_lfind" "$URI" -perm "-${perms[i]}" -type f | wc -l)
        (( "$num" == "${perm_minus[i]}" )) ||
            error "$rbh_lfind -perm -${perms[i]}: $num != ${perm_minus[i]}"

        num=$("$rbh_lfind" "$URI" -perm "/${perms[i]}" -type f | wc -l)
        (( "$num" == "${perm_slash[i]}" )) ||
            error "$rbh_lfind -perm /${perms[i]}: $num != ${perm_slash[i]}"
    done
}
...
function test_symbolic
{
    ...
    for (( i = 0; i < ${#symbolic[@]}; i++ )); do
        local num=$("$rbh_lfind" "$URI" -perm "${symbolic[i]}" | wc -l)

        (( $num == 1 )) ||
            error "$rbh_lfind $URI -perm ${symbolic[i]}: $num != 1"
    done
}
...
function test_error_perm
{
    ...
    for err in "${tests[@]}"; do
        if "$rbh_lfind" "$URI" -perm "$err"; then
            error "$rbh_lfind -perm $err: parsing should have failed"
        fi
    done
}
