...
test_expired_rel()
{
    ...
    rbh_lfind "rbh:mongo:$testdb" -expired -$(date +%s --date='1 hour') | sort |
        difflines "/$fileA" "/$fileB" "/$fileC"
    rbh_lfind "rbh:mongo:$testdb" -expired \
        $(($(stat --format='+%X' "$fileA") + timeA)) | sort |
        difflines "/$fileA" "/$fileB"
    rbh_lfind "rbh:mongo:$testdb" \
        -expired +$(date +%s) -expired -$(date +%s --date='1 minute') | sort |
        difflines "/$fileC"
    rbh_lfind "rbh:mongo:$testdb" \
        -expired +$(date +%s --date='-1 hours') -expired -$(date +%s) | sort |
        difflines "/$fileA" "/$fileB"
}
