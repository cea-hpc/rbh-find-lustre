...
lustre_str2command_line_token(const char *string)
{
    switch (string[0]) {
    case '-':
...
main(int _argc, char *_argv[])
{
    ...
    ctx.str2command_line_token_callback = &lustre_str2command_line_token;

    /* Parse the command line */
    for (index = 0; index < ctx.argc; index++) {
        if (lustre_str2command_line_token(ctx.argv[index]) != CLT_URI)
            break;
    }
    if (index == 0)
